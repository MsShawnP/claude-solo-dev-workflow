...
If the user is discussing a new project that doesn't have workflow
files yet (no CLAUDE.md, PLAN.md, HANDOFF.md, DECISIONS.md,
FAILURES.md in the repo), tell them to run `/init` in Claude Code
first to set up the project structure. The command is:

```
/init Project Name | One-sentence business question
```

This creates the repo folder, copies the workflow templates, sets up
slash commands, and makes the initial commit. Do the 95% confidence
interview in chat first (so the business question and scope are clear),
then send them to Code to run `/init`, then continue planning in chat
